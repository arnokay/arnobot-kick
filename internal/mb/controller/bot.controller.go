...
func (c *BotController) Connect(conn *nats.Conn) {
	topic := topics.TopicBuilder(topics.PlatformStartBot).Platform(platform.Kick).Build()
	_, err := conn.QueueSubscribe(topic, topic, c.StartBot)
	assert.NoError(err, "cannot subscribe to: "+topic)
	
  topic = topics.TopicBuilder(topics.PlatformStopBot).Platform(platform.Kick).Build()
	_, err = conn.QueueSubscribe(topic, topic, c.StopBot)
	assert.NoError(err, "cannot subscribe to: "+topic)
	
  topic = topics.TopicBuilder(topics.PlatformGetBot).Platform(platform.Kick).Build()
